import React from "react";
import { useApiQuery } from "../../hooks/api/useApiQuery";
import ReusableCard from "../reusable/ReusableCard";
import ReusableTable from "../reusable/ReusableTable";
import ReusableLoader from "../reusable/ReusableLoader";

/**
 * RemoveFromBulkPayment Component
 * - Uses reusable components for all UI elements
 * - API data is fetched using generic useApiQuery hook (React Query)
 * - No direct axios or Redux logic
 * - SOLID/DRY principles, robust error handling, maintainable code
 * - Detailed comments for major functionalities
 */
function RemoveFromBulkPayment() {
  // Fetch bulk payment data using generic React Query hook
  const {
    data: bulkPaymentData,
    error,
    isLoading,
  } = useApiQuery({
    key: "bulk-payment",
    url: "/api/v1/bulk-payment", // Replace with actual endpoint
    method: "get",
    enabled: true,
    select: (data) => data?.payments || [], // Transform response if needed
    retry: 2,
    staleTime: 60000,
    cacheTime: 300000,
  });

  // Table columns definition for ReusableTable
  const columns = [
    { Header: "Payment ID", accessor: "id" },
    { Header: "Customer Name", accessor: "customerName" },
    { Header: "Amount", accessor: "amount" },
    {
      Header: "Status",
      accessor: (row) => (row.status ? "Active" : "Inactive"),
    },
  ];

  return (
    <div className="work-space-container">
      <ReusableCard title="Remove From Bulk Payment">
        {/* Loader and error handling */}
        {isLoading && <ReusableLoader />}
        {error && (
          <p className="text-danger">Error: {error.message || error}</p>
        )}
        {/* Table display */}
        {!isLoading && !error && (
          <ReusableTable
            columns={columns}
            data={bulkPaymentData || []}
            striped
            bordered
            responsive
            className="table table-bordered table-striped"
          />
        )}
        {/* If no data available */}
        {!isLoading &&
          !error &&
          (!bulkPaymentData || bulkPaymentData.length === 0) && (
            <p className="text-warning">No bulk payment data available.</p>
          )}
      </ReusableCard>
    </div>
  );
}

export default RemoveFromBulkPayment;
