...
function LoadingReport() {
  ...
  const [billingPartyOptions, setBillingPartyOptions] = useState([]);
  const [consignorOptions, setConsignorOptions] = useState([]);
  const [consigneeOptions, setConsigneeOptions] = useState([]);
  const [displayOrderOptions, setDisplayOrderOptions] = useState([]);
  ...
  useEffect(() => {
    const fetchSelectOptions = async () => {
      try {
        const billingPartyRes = await axiosInstance.get(
          "/api/v1/get/all/trader-billing-party-names"
        );
        const billingPartyArrayOfObjects = billingPartyRes.data.map(
          (element) => {
            return {
              id: element[0],
              name: element[1],
        ...
        setBillingPartyOptions(billingPartyArrayOfObjects);

        const consignorRes = await axiosInstance.get(
          "/api/v1/get/all/loading-point-names"
        );
        const consignorResArrayOfObjects = consignorRes.data.map((element) => {
          return {
            id: element[0],
            name: element[1],
          };
        });
        setConsignorOptions(consignorResArrayOfObjects);

        const consigneeRes = await axiosInstance.get(
          "/api/v1/get/all/unloading-point-names"
        );

        const consigneeResArrayOfObjects = consigneeRes.data.map((element) => {
          return {
            id: element[0],
            name: element[1],
          };
        });
        setConsigneeOptions(consigneeResArrayOfObjects);

        const displayOrderRes = await axiosInstance.get(
          "/api/v1/get/all/unloading-point-names"
        );
        const displayOrderResArrayOfObjects = displayOrderRes.data.map(
          (element) => {
            return {
              id: element[0],
              name: element[1],
        ...
        setDisplayOrderOptions(displayOrderResArrayOfObjects);
      } catch (error) {
  ...
  async function postFilteredData(data, page) {
    const searchedData = {
      "billingParty": data.billingParty ? data.billingParty : null,
      "consignor": data.consignor ? data.consignor : null,
      "consignee": data.consignee ? data.consignee : null,
      "loadingPoint": data.loadingPoint ? data.loadingPoint : null,
      "unloadingPoint": data.unloadingPoint ? data.unloadingPoint : null,
     ...
     " endDate": data.permitToDate
        ...
        : null,
      "displayOrder": data.displayOrder ? data.displayOrder : null,
      page: page,
      // "size": pageSize,
    };
    ...
    <div>
      <div className="work-space-container">
        <div
          ...
          role="alert"
        >
          <span className="mb-0 h6">Query On Load Challan</span>
        </div>
          ...
          <div className="form-row">
            <div className="form-group col-md-2">
              ...
              >
                ...
                {billingPartyOptions.map((option) => (
